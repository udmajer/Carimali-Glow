...
English, Italian, German and Spanish pages are available at `/en/glow/`, `/it/glow/`, `/de/glow/` and `/es/glow/`. The generation and SEO structure is documented in `LOCALIZATION.md`. The shared form code maps Italian pages to `lang: "it"` and every other language to `lang: "en"`, as required by the lead API.
